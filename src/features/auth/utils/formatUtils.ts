...
export const validationFormatters = {
  ...
  formatValidationMessage: (field: string, rule: string, value?: any): string => {
    const messages: Record<string, string> = {
      'required': `${field} é obrigatório`,
      'email': `${field} deve ser um e-mail válido`,
      'minLength': `${field} deve ter pelo menos ${value} caracteres`,
      'maxLength': `${field} deve ter no máximo ${value} caracteres`,
      'pattern': `${field} tem formato inválido`,
      'min': `${field} deve ser pelo menos ${value}`,
      'max': `${field} deve ser no máximo ${value}`,
      'confirm': `${field} não confere`
    };

    return messages[rule] || `${field} é inválido`;
  },

  // Formatar lista de erros
...
export const formatters = {
  ...
  // Privacidade
  privacy: privacyFormatters,

  // Função helper para uso geral
  format: (type: string, value: any, options?: any): string => {
    switch (type) {
      case 'email':
        return emailFormatters.normalize(value);
      case 'phone':
        return phoneFormatters.formatBrazilian(value);
      case 'name':
        return nameFormatters.formatFullName(value);
      case 'date':
        return dateFormatters.formatDate(value, options);
      case 'error':
        return errorFormatters.formatUserError(value);
      default:
        return String(value);
    }
  }
